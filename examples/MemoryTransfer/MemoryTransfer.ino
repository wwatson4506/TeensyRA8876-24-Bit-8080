...
void drawBG(int x, int y, int w, int h) {
  ...
  tft.fillRect(x, y, w, h, COLOR16M_BLACK);
  tft.check2dBusy();
  ...
  tft.fillRect(x + 3, y + 3, w - 6, h / 3, COLOR16M_WHITE);
  tft.fillRect(x + 3, y + 3 + h / 3, w - 6, h / 3, COLOR16M_LIGHTBLUE);
  tft.fillCircle(x + w / 2, y + h / 2, h / 4, COLOR16M_LIGHTRED);
  tft.fillTriangle(x + 2 * w / 8, y + 3 * h / 8, x + 1 * w / 8, y + 5 * h / 8, x + 3 * w / 8, y + 5 * h / 8, COLOR16M_DARKGREEN);
  tft.fillRect(x + 5 * w / 8, y + 3 * h / 8, w / 4, h / 4, COLOR16M_DARKYELLOW);
}
...
void writeImageChromakeyZoom(int x, int y, int w, int h, ru16 chromakeyColor, int magnification, const unsigned char *image) {
  ...
  tft.fillRect(x, y, w * magnification, h * magnification, COLOR16M_MAGENTA);
...
void setup() {
  ...
  tft.fillScreen(COLOR16M_DARKBLUE);
  ...
  tft.setTextColor(COLOR16M_WHITE, COLOR16M_DARKBLUE);
...
//    Serial.print((float)(end2 - endTime) / 1000.0, 3);
  ...
  tft.fillScreen(COLOR16M_DARKGREEN);
  tft.setCursor(400, 180);
  tft.setTextColor(COLOR16M_WHITE, COLOR16M_DARKGREEN);
